...
export default new Command({
  ...
  run: async (client, interaction) => {
    ...
    let rankcard = new MessageAttachment(
      `https://express-api.ml/api/rankcard?username=${target.displayName}&avatar=${avatar}&level=${level}&rank=${position}&currentXp=${xp}&xpNeeded=${NextLevelxp}&background=${background}&boosting=${booster}&staff=${staff}&bar=${barColor}&circle=${AvatarShape}`,
      'rankcard.png'
    );
